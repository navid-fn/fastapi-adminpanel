...
async def create_user(user: UserCreate, session: Session = Depends(get_session)):
    if session.exec(select(User).where(or_(User.email == user.email, User.username == user.username))).first():
...
async def read_active_users(session: Session = Depends(get_session), offset: int = 0, limit: int = 100):
